...
export const getCardValue = (cardValue) => (isNumber(cardValue) ? +cardValue : cardValue[0] === 'A' ? 1 : 10);
...
export const getPoints = (cards) => {
	...
	console.log(`(${visibleCards.length}) cards shown = ${sumOfCardsDisplayed} pts`);
...
export const updateItem = (arr, itemId, newItem) => {
	...
	const updatedArray = arr.map((item) => (item.code === itemId ? { ...item, ...newItem } : item));

...
import { Page, Header, ButtonLink, Content, TextContent } from '../styles/Layout';
...
class Game extends Component {
	...
	start = () => {
		...
		getDeckId().then(({ deck_id }) => getFirstCards(deck_id, startingCards)).then(this.setCardsInGame);
	};
		...
		return (
			<div>
				{gameOver ? (
					...
					<Page className="game">
						{remaining > 0 && <TextContent> {remaining} cards left in deck </TextContent>}
						<Hand
							roleName="player"
							cards={playerPile}
							points={playerPoints}

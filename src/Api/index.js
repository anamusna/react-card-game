...
export const getFirstCards = (deck_id, startingCards = '10') => {
	return fetch(`${API_BASE_URL}${deck_id}/draw/?count=${startingCards}`).then((r) => r.json()).then((data) => {
		const newState = {
...
export const drawCard = (deck_id, amount = 2) => {
	return fetch(`${API_BASE_URL}${deck_id}/draw/?count=${amount}`).then((r) => r.json()).then((data) => {
		const newState = {
			remaining: data.remaining,
			cards: turnFaceDown(data.cards)
		};
